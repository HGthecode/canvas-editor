...
export class ContextMenu {
  ...
  private _filterMenuList(menuList: IRegisterContextMenu[]): IRegisterContextMenu[] {
  ...
  private _getContext(evt: MouseEvent): IContextMenuContext {
    ...
    if (
      ...
      typeof positionResult.tdValueIndex === 'number' &&
      positionResult.tdValueIndex > -1
    ) {
      index = positionResult.tdValueIndex
    } else if (
      positionResult &&
      typeof positionResult.index === 'number' &&
      positionResult.index > -1
    ) {
      index = positionResult.index
    }
    let hoverElement = elementList[index]
    ...
    if (hoverElement && hoverElement.type === ElementType.TABLE && hoverElement.trList) {
      ...
      const trList = hoverElement.trList[trIndex]
      const tdIndex = positionResult?.tdIndex as number
      const tdList = trList.tdList[tdIndex]
      if (
        tdList.value &&
        typeof positionResult?.tdValueIndex === 'number' &&
        positionResult.tdValueIndex > -1
      ) {
        hoverElement = tdList.value[positionResult.tdValueIndex]
      }
      hoverIndex = positionResult?.tdValueIndex as number
    } else if (!hoverElement && startElement && startElement.type === ElementType.CONTROL) {
      // hoverElement = startElement
    }
  ...
  private _render(payload: IRenderPayload): HTMLDivElement {
    ...
    for (let c = 0; c < contextMenuList.length; c++) {
      ...
      if (menu.isDivider) {
        // 分割线相邻 || 首尾分隔符时不渲染
        if (c !== 0 && c !== contextMenuList.length - 1 && !contextMenuList[c - 1]?.isDivider) {
